...
const ElementLibrary: React.FC = () => {
  ...
  const elements: { type: ElementType; subtype?: string; icon: React.ReactNode; label: string }[] = [
  ...
  const handleElementClick = (type: ElementType, subtype?: string) => {
    ...
    // Store subtype if provided
    if (subtype) {
      (window as any).__elementSubtype = subtype;
    } else {
      delete (window as any).__elementSubtype;
    }
    setShowDialog(true);
  };
  ...
  const handleAddElement = () => {
    if (elementTypeToAdd && selectedRowId && selectedColumnId) {
      const subtype = (window as any).__elementSubtype;
        ...
        {elements.map((element, index) => (
          ...
          >
            <CardContent className="p-0">
              <Button
                variant="ghost"
                onClick={() => handleElementClick(element.type, element.subtype)}
          ...
          {getAllElements().map(element => (
            ...
            >
              <div className="flex items-center gap-2">
                ...
                {element.type === 'input' && element.inputType === 'checkbox' && <CheckSquare className="h-4 w-4" />}
                {element.type === 'input' && element.inputType === 'radio' && <Check className="h-4 w-4" />}
                {element.type === 'input' && element.inputType === 'select' && <ListOrdered className="h-4 w-4" />}
                {element.type === 'input' && !['checkbox', 'radio', 'select'].includes(element.inputType || '') && <Pencil className="h-4 w-4" />}
                {element.type === 'image' && <ImageIcon className="h-4 w-4" />}
                <span className="truncate max-w-[180px]">
                  {element.type === 'text' && element.content}
                  {element.type === 'button' && element.label}
                  {element.type === 'input' && (element.label || element.placeholder || element.inputType)}
